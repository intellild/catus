...
│                                                                             │
│   ┌─────────────────────────────────┐    ┌─────────────────────────────┐   │
│   │     PTY Read Task               │    │     run_terminal_loop       │   │
│   │  ┌─────────────────────────┐    │    │  ┌───────────────────────┐  │   │
│   │  │  pty.start_reader()     │    │    │  │ loop {                │  │   │
│   │  │       │                 │    │    │  │   match input_rx {    │  │   │
│   │  │       ▼                 │    │    │  │     Write(data) => {  │  │   │
│   │  │  while let Some(data) { │    │    │  │       pty.write(data) │  │   │
│   │  │    input_tx.send(       │────┼────┼─►│     }                 │  │   │
...
terminal.input(data)
    │
    ▼
input_tx.send(TerminalInput::Write(data))
    │
    ▼
run_terminal_loop: pty.write(data)  // 后台任务中执行
    │
    ▼
Shell 进程
```
...
- PTY 通过 `Terminal::new(pty, cx)` 直接传入，所有权转移到后台任务
- `LocalPty` 使用 `Arc<Mutex<_>>` 实现内部可变性，支持 `&self` 方法
- `LocalPty` 保存读取线程 handle 和子进程 handle，Drop 时自动清理
