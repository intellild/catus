...
- `src/terminal/fake_pty.rs`: 仅测试构建可用的 `FakePty`，实现 `Pty` trait；写入只记录不回显，可向 reader 注入输出用于模拟程序输出/OSC 序列；`close_reader` 可模拟 PTY EOF。需要验证 echo 行为时使用 `scripts/echo-pty.js` 作为真实 Node 子进程。
