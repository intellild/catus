...
- **Multiple workspaces** — a left sidebar lists all workspaces; switch, close, or add new ones.
  ...
  - **tmux** workspaces use control mode to show server windows and panes natively. Select **tmux** in Add Workspace and enter `tmux -CC new-session -A -s work`; see [tmux support](docs/tmux-control-mode.md) for SSH, lifecycle and current limits.
